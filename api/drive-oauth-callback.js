...
async function handler(req, res) {
  ...
  if (!verifyState(state)) {
    return sendText(res, 401, 'Invalid or expired OAuth state. Start again from /api/drive-oauth?action=connect.');
  }
  ...
  try {
    ...
    return sendHtml(res, 200,
      ...
      '<p>After updating Vercel, check <code>/api/drive-oauth?action=health</code> with the fulfillment retry token.</p>' +
      '</body>'
    );
  } catch (err) {
