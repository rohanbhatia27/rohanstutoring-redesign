...
function getModel() {
  // Use the fast NON-thinking model by default. The "pro"/thinking model spends
  // its whole token budget on reasoning_content (leaving content empty) and is
  // slow enough to hit the serverless timeout. Flash is plenty for structured JSON.
  return String(process.env.DEEPSEEK_INSIGHTS_MODEL || 'deepseek-v4-flash').trim();
}

function getMaxTokens() {
  return Number(process.env.AI_INSIGHTS_MAX_TOKENS) || 4000;
}
